...
from utils.util_print import STR_STAGE
...
def cameraList_from_camInfos(cam_infos, resolution_scale, args, mode="test") -> List[Camera]:
...
def focus_point_fn(poses: np.ndarray) -> np.ndarray:
    ...
    # 计算相机的焦点点 也就是所有相机焦点轴的最近公共点
    directions, origins = poses[:, :3, 2:3], poses[:, :3, 3:4]  # 计算所有相机的射线方向和中心点 也就是z轴向量和中心偏移向量
    m = np.eye(3) - directions * np.transpose(directions, [0, 2, 1])  # I - 投影矩阵 == 正交投影矩阵M 也就是投影到和direction垂直的平面
    mt_m = np.transpose(m, [0, 2, 1]) @ m  # 求解正规方程的其中一步
...
def generate_ellipse_path_from_poses(poses: np.ndarray,
                          ...
                          z_variation: float = 0.,  # 振幅
                          z_phase: float = 0.) -> np.ndarray:  # phi of A * cos(w*t + phi)
    """Generate an elliptical render path based on the given poses."""
    # 类似于NeRF中的spherify_poses()函数
    # Calculate the focal point for the path (cameras point toward this).
    center = focus_point_fn(poses)  # 找到相机的焦点点 也就是整个场景的中心
    # Path height sits at z=0 (in middle of zero-mean capture pattern).
    offset = np.array([center[0], center[1], 0])  # 场景中心在2D平面的直接投影

    # Calculate scaling for ellipse axes based on input camera positions.
    # 先计算每个相机位置距离2D中心点的距离 然后计算上述距离沿轴向的 100 百分位数 也就是相机的最远分布 可以直接当做椭圆的半径
    sc = np.percentile(np.abs(poses[:, :3, 3] - offset), 100, axis=0)
    # Use ellipse that is symmetric about the focal point in xy.
    # 画个图帮助理解 offset表示场景中心距离世界原点的向量 sc表示场景的最长长度 对offset这个场景中心加减一个sc 半长轴和半短轴的世界坐标下的向量
    low = -sc + offset
    high = sc + offset
    # Optional height variation need not be symmetric
    # 扩展到3D
    # 用于确定z轴也就是3D椭圆的高度的上下界
    ...
    def get_positions(theta):
        ...
        return np.stack([
            low[0] + (high - low)[0] * (np.cos(theta) * .5 + .5),          # x: 使用余弦函数来在x方向上插值
            low[1] + (high - low)[1] * (np.sin(theta) * .5 + .5),          # y: 使用正弦函数来在y方向上插值 
            z_variation * (z_low[2] + (z_high - z_low)[2] *
                        (np.cos(theta + 2 * np.pi * z_phase) * .5 + .5)),  # z: 使用余弦函数来在z方向上插值 加入振幅和相位改变
        ], -1)  # 使用sin-cos生成椭球形的相机坐标 再次强调的是必须经过前面的归一化过程才有意义

    theta = np.linspace(0, 2. * np.pi, n_frames + 1, endpoint=True)
    positions = get_positions(theta)
    print(STR_STAGE, f'In utils/camera_utils.py: Generating {len(theta)} ellipse positions theta[0]: {theta[0]}, theta[1]: {theta[1]}, theta[2]: {theta[2]}, theta[3]: {theta[3]}, ...')

    if const_speed:
        # Resample theta angles so that the velocity is closer to constant.
        print(STR_STAGE, "Resample theta angles so that the velocity is closer to constant...")
    ...
    avg_up = poses[:, :3, 1].mean(0)  # 拿到y轴对应的旋转向量  因为viewmatrix中up向量对应的就是y轴
...
def transform_poses_pca(poses: np.ndarray) -> Tuple[np.ndarray, np.ndarray, float]:
    """Transforms poses so principal components lie on XYZ axes.
        使得变换后的pose矩阵和XYZ对齐 且位置向量被缩放到了[-1, 1]立方体内 相当于把相机位置进行约束和归一化了
        主要作用是normalize data for simplifying subsequent
    ...
    t = t - t_mean  # 中心化相机在世界坐标系中的位置  其实是保证了相机中心点就在世界坐标原点

    # PCA
    eigval, eigvec = np.linalg.eig(t.T @ t)  # 找到principal components of translation
    # Sort eigenvectors in order of largest to smallest eigenvalue.
    inds = np.argsort(eigval)[::-1]  # 按特征值大小对特征向量进行排序
    eigvec = eigvec[:, inds]
    rot = eigvec.T  # 旋转矩阵就是特征向量
    if np.linalg.det(rot) < 0:
        rot = np.diag(np.array([1, 1, -1])) @ rot  # 行列式为负 表示reflection 需要调整来保证右手坐标系

    transform = np.concatenate([rot, rot @ -t_mean[:, None]], -1)  # 构建出完整的transform matrix
    poses_recentered = unpad_poses(transform @ pad_poses(poses))  # 先pad换成齐次坐标方便transform 然后再unpad回来
    transform = np.concatenate([transform, np.eye(4)[3:]], axis=0)  # 齐次坐标形式的transform

    # Flip coordinate system if z component of y-axis is negative  如果y轴在z方向上投影为负 说明y轴被翻转了
    if poses_recentered.mean(axis=0)[2, 1] < 0:
        poses_recentered = np.diag(np.array([1, -1, -1])) @ poses_recentered  # 整个相机的pose全部翻转
        transform = np.diag(np.array([1, -1, -1, 1])) @ transform  # 对应的transform matrix也要翻转

    # Just make sure it's it in the [-1, 1]^3 cube  把点云/相机位姿缩放到标准化的立方体内 用于规范化数据
    scale_factor = 1. / np.max(np.abs(poses_recentered[:, :3, 3]))  # 拿到scale
    poses_recentered[:, :3, 3] *= scale_factor # translation那一列进行scale  其实就是把相机中心全部扔到一个[-1, 1]^3的立方体内

    return poses_recentered, transform, scale_factor

def invert_transform_poses_pca(poses_recentered, transform, scale_factor):
    # 对相机坐标取消缩放、逆变换和从齐次坐标表示还原
    poses_recentered[:, :3, 3] /= scale_factor  # 偏移向量除以scale 相当于还原回来之前的偏移量
    transform_inv = np.linalg.inv(transform)  # 中心化后坐标变为原始坐标
...
def generate_ellipse_path_from_camera_infos(
    ...
    # 生成环形的相机positions列表
    render_poses = generate_ellipse_path_from_poses(poses, n_frames, const_speed, z_variation, z_phase)  # 生成椭球形分布的相机位姿 共120个点
    # 标准空间变换回原始的相机位姿
    render_poses = invert_transform_poses_pca(render_poses, transform, scale_factor)
    render_poses[:, :, 1:3] *= -1  # 接着反向恢复 最后得到环形位姿的相机positions
    ret_cam_infos = []
    for uid, pose in enumerate(render_poses):  # 对环形的相机轨迹生成场景信息
        R = pose[:3, :3]
        c2w = np.eye(4)
        c2w[:3, :4] = pose
