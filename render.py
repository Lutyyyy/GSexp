...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, save_images=True, not_generate_video=False):
    ...
    lpipss = 0.
    depths = []
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")): # 遍历所有相机视图渲染
    ...
    if not not_generate_video:
        ...
        subprocess.run(["ffmpeg", "-y", 
                    ...
                    "-c:v", "libx264", 
                    "-crf", "23", 
                    # "-pix_fmt", "yuv420p",  # Set pixel format for compatibility  #? YUV颜色空间
                    renders_path], 
                    stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
                    )  # render rendered_img
        subprocess.run(["ffmpeg", "-y", # -y 覆写输出文件时不需要提示确认
                    "-framerate", "24",  # 帧率 24
                    "-i", os.path.join(gts_path, "%05d.png"), 
                    "-vf", "pad=ceil(iw/2)*2:ceil(ih/2)*2",  # 添加边缘填充
                    "-c:v", "libx264",  # 指定视频编解码器为 libx264
                    "-crf", "23", # 常量速率因子
                    # "-pix_fmt", "yuv420p",  # Set pixel format for compatibility(兼容性)
                    gt_path], # 输出路径
                    stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL  # 不输出错误到控制台
                    )  # render gt_img
        ...
        for depth in depths:
            ...
            if depth.max() > 0:
                ...
                depth_normalized = cv2.normalize(depth, depth, 0.0, 1.0, cv2.NORM_MINMAX) # 归一化到[0, 1]
            else:
                depth_normalized = np.zeros_like(depth)
            depth_normalized = cv2.normalize(depth, None, 0, 255, cv2.NORM_MINMAX)  # 归一化到[0, 255]
...
def render_path(dataset : ModelParams, iteration : int, pipeline : PipelineParams, extra_opts=None):
    ...
    views = scene.getRenderCameras()  # 环形相机路径
    ...
    # Use ffmpeg to output video
    renders_path = os.path.join(model_path, name, "ours_{}".format(iteration), "renders_path.mp4")
    # Use ffmpeg to output video
    subprocess.run(["ffmpeg", "-y", 
                "-framerate", "24",
                "-i", os.path.join(render_path, "%05d.png"), 
...
if __name__ == "__main__":
    ...
    if args.is_eval:
        with torch.no_grad():
            evaluate([args.model_path])  # 单纯计算metric
        exit()
