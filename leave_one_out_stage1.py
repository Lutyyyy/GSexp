...
from utils.util_print import STR_STAGE, STR_VERBOSE
...
def leave_one_out_training(args, dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, train_id):
    ...
    #NOTE 此时已经使用了visual_hull_4.ply点云
    scene = Scene(dataset, gaussians, shuffle=False, extra_opts=args)  # make sure no shuffle
    gaussians.training_setup(opt)
    if checkpoint:
        #?NOTE 没有用上CoarseGS的点云
        print(STR_VERBOSE + "resuming the gaussian......")
    ...
    for iteration in range(first_iter, opt.iterations + 1):  # [1 ~ 10000]
        ...
        with torch.no_grad():
            ...
            # Save
            if iteration in saving_iterations:  # [70000, 150000, 10000]
                #NOTE 只会在最后结束训练的时候保存下GS 存在point_cloud/iteration_10000/point_cloud.ply中
            ...
            if iteration in checkpoint_iterations:
                # 每6000轮训练存一次模型 后面结束训练再存一次模型
                print("\n[ITER {}] Saving Checkpoint".format(iteration))
                torch.save((gaussians.capture(), iteration), scene.model_path + "/chkpnt" + str(iteration) + ".pth")  # just for resuming, nothing uselful

            # cache the rendered images in the rendering progress. (6000个轮次后开始存下leave-one图片的渲染图 此时已经不再densitify了)
    ...
    torch.save(gaussians.cache, os.path.join(args.model_path, "gaussians_cache.pth"))  # sec 3.4 \G_c^i_bar
...
def prepare_output_and_logger(args):
    ...
    print("Output folder: {}".format(args.model_path))  # model_path == img_id
...
def cal_loss(opt, args, image, render_pkg, viewpoint_cam, bg, silhouette_loss_type="bce", mono_loss_type="mid"):
    ...
    if hasattr(viewpoint_cam, "mono_depth") and viewpoint_cam.mono_depth is not None:
        if mono_loss_type == "mid":
            ...
            # 对zoe_depth做不同处理并和rendered_depth做比较
            depth_loss = min((1 - pearson_corrcoef(-zoe_depth, rendered_depth)), (1 - pearson_corrcoef(1 / (zoe_depth + 200.0), rendered_depth)))
        else:
            raise NotImplementedError
...
def train_3dgs(args, ids):
    print(STR_STAGE + "Optimizing {args.model_path}")
    ...
    for num_id, image_id in zip(range(args.sparse_view_num), ids):  # num_id: leave one out id, image_id: the id of the image to be infered
        ...
        os.makedirs(args.model_path, exist_ok=True)
        leave_one_out_training(
            args, dataset, op.extract(args), pipeline, args.test_iterations, args.save_iterations, args.checkpoint_iterations, args.start_checkpoint, args.debug_from, train_id=(num_id, image_id)
        )
...
if __name__ == "__main__":
    ...
    args.save_iterations.append(args.iterations)  # 最后一步也得加入保存场景信息
