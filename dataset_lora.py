...
from utils.util_print import STR_VERBOSE, STR_DEBUG
...
class GSCacheDataset(Dataset):
    def __init__(
        ...
        cache_max_iter: int = 240,  # default in dataset_lora is 100
        train: bool = True
    ):
        super().__init__()
        self.gaussian_dir = gaussian_dir  # train_gs.py ==> CoarseGS
        self.data_dir = data_dir
        self.loo_dir = loo_dir
        self.image_size = image_size  #NOTE 512
        ...
        ply_path = os.path.join(self.gaussian_dir, 'point_cloud', f'iteration_{self.iter}', 'point_cloud.ply')  # CoarseGS的点云
        ...
        if self.resolution in [1, 2, 4, 8]:
            ...
            if not os.path.exists(tmp_images_folder):
                ...
                print(STR_VERBOSE, f"Using resized images in {tmp_images_folder}...")
        ...
        self.statistics_info:list[dict] = []  # binary file .pkl saves a dict

        for idx, extr in enumerate(cam_extrinsics):
            if (self.train and idx in sparse_ids) or (not self.train and idx not in sparse_ids):  # 要么训练集要么测试集
                intr = cam_intrinsics[extr.camera_id]
                height = intr.height  # 2078
                width = intr.width    # 3115
                ...
                if self.train:
                    noisy_paths = os.listdir(os.path.join(self.loo_dir, f'leave_{idx}', 'left_image'))  # 6000~10000轮次渲染出来的one-image
                    its = sorted([int(path.replace('sample_', '').replace('.png', '')) for path in noisy_paths])  # 按照图片名称排序
                    min_it = its[0]  # 6004
                    # noisys就是6000~10000那一张one图片
                    ...
                    print(STR_DEBUG, f'Load {len(noisys)} images for leave {idx}')  # 25. 因为是 100的cache_max_iter / 4张图片
    ...
    def center_pad(self, image: Union[np.ndarray, torch.Tensor]) -> Union[np.ndarray, torch.Tensor]:  # 填充成正方形 image_size
        H, W, _ = image.shape  #NOTE (520, 779, 3)
    ...
    def center_crop(self, image: Union[np.ndarray, torch.Tensor]) -> Union[np.ndarray, torch.Tensor]:  # 裁剪成正方形 image_size
        image = self.center_pad(image)  # 先填充成正方形
        H, W, _ = image.shape
    ...
    def resize_image(self, input_image: np.ndarray) -> np.ndarray:  # 调整到最近的64的倍数大小
    ...
    def __getitem__(self, idx):
        ...
        if not self.train:
            ...
            source = self.center_crop(noisy.clamp(0., 1.).permute(1, 2, 0))  # 直接用当前干净高斯渲染出来的裁剪图片
        elif random() < self.manual_noise_prob:  # 如果小于1 则手动设置噪声
            ...
            render_pkg = render(cam, self.gaussian, self.pipe, self.bg_color)
            noisy = render_pkg['render']
            self.gaussian.restore_noise()  # 恢复原先的没有噪声的GS属性
            source = self.center_crop(noisy.clamp(0., 1.).permute(1, 2, 0))  # 加噪GS渲染出来的图片
        else:
            noisy = choice(self.noisys[idx])  # 随便选一张leave-one的图片
        ...
        return {
            'jpg': target,  # 原始data/mip360图像
            'txt': choice(imagenet_templates_small).format(self.prompt) if self.use_prompt_list else f'a photo of a {self.prompt}',  # prompt
            'hint': source  #NOTE CoarseGS加噪图片/CoarseGS渲染出来的图片/渲染出来的半留一法训练出来的leave-one图片
        }
