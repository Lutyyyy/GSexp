...
class GaussianModel:
    ...
    def densify(self, max_grad, extent):  #NOTE for GSDreamer
        ...
        self.logger.info(f'In scene/gaussian_model.py: grads.max(): {grads.max()}, grads.min(): {grads.min()}')
        self.densify_and_clone(grads, max_grad, extent)
        self.densify_and_split(grads, max_grad, extent)

    def prune(self, min_opacity, extent, max_screen_size):  #NOTE for GSDreamer
        prune_mask = (self.get_opacity < min_opacity).squeeze()
        for_opacity = prune_mask.sum()
        if max_screen_size:
        ...
        self.logger.info("In scene/gaussian_model.py: Pruning: {} points removed, Specifically, for opacity {} points, for big {} points in vs and {} points in ws, max_screen_size is {}".format(prune_mask.sum(), for_opacity, for_bigpoints_vs, for_bigpoints_ws, max_screen_size))
        self.prune_points(prune_mask)
    ...
    def remove_outliers(self, opt, step, linear=False, removing_ratio=0, remaining_rate=0.0):  # 为了区分object和floater
        ...
        if not linear:  # 不使用线性增长的阈值 z-score为正则表示距离均值有多少个标准差的距离 为负则直接变成一个标准差
            # 直接根据传入的比例参数去除离群点
            ...
            # 点云越密集 点就越近 所以 std_knn + mean_knn 越小
            # 也就是 lambda_sigma 从 -inf -> 2.3263478740408408 具体根据step线性调整去除离群点的比例
        ...
        nearest_k_distance = p3dops.knn_points(
            xyz.unsqueeze(0),
            xyz.unsqueeze(0),
            K=int(num_points**0.5),  #NOTE eq. sqrt(p)
        ).dists  # 经验法则选择 sqrt(p)
    ...
    def restore_noise(self):  #NOTE for LoRA
        # Restore the attributes from backup
        for attr, value in self._backup_attributes.items():
            setattr(self, attr, value)

    def add_statistics_noise(self, statistics_info: list[dict], noise_dropout: float = 0., std_scale: float = 0.7):  #NOTE for LoRA
        ...
        for key in statistics_info[0].keys():
            means[key] = np.mean([info[key][0] for info in statistics_info], axis=0)  # 取四张稀疏图片构建的\delta的均值
            stds[key] = np.mean([info[key][1] for info in statistics_info], axis=0)

        # Backup and add noise
        for attr in attributes_to_noise:
            self._backup_attributes[attr] = getattr(self, attr)  # 先存下没加噪的GS属性方便恢复
            if attr in ['_xyz', '_scaling', '_rotation', '_opacity']:  # 对mean, variance, opacity加噪 所有属性用的是同一套噪声
                ...
                noise = torch.randn_like(getattr(self, attr)) * cur_std + cur_mean  # 生成噪声
                if attr == '_scaling' or attr == '_opacity' or attr == '_rotation':  #? 不对位置进行裁剪?
                    noise = torch.clamp(noise, cur_mean - std_scale * cur_std, cur_mean + std_scale * cur_std)  #? 裁剪噪声到固定范围 裁剪的范围如何得出来?

                noise[torch.rand_like(getattr(self, attr)) < noise_dropout] = 0  # 随机丢弃噪声
                setattr(self, attr, getattr(self, attr) + noise)  # 把噪声加到GS属性上
