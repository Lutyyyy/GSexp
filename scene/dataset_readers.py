...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder, extra_opts=None):
    ...
    if extra_opts.resolution in [1, 2, 4, 8]:
        tmp_images_folder = images_folder + f'_{str(extra_opts.resolution)}' if extra_opts.resolution != 1 else images_folder  # 选好下采样的文件夹
...
def readColmapSceneInfo(path, images, eval, llffhold=8, extra_opts=None):
    ...
    if eval:
        train_cam_infos: List[CameraInfo] = [c for idx, c in enumerate(cam_infos) if idx % llffhold != 0]  # 279 images in kitchen
    ...
    if hasattr(extra_opts, 'sparse_view_num') and extra_opts.sparse_view_num > 0: # means sparse setting
        ...
        print(STR_VERBOSE + f"Sparse view, only {len(ids)} images are used for training, others are used for eval.")

    #NOTE in sparse condition, we may use random points to initialize the gaussians
    #NOTE not implemented for random initialize
    ...
    if hasattr(extra_opts, 'transform_the_world') and extra_opts.transform_the_world:
        ...
        MLMatrix44 = load_meshlab_file(osp.join(path, "pcd_transform.txt"))  # 加载一个4*4的变换矩阵
        ...
        pcd = transform_pcd(pcd, MLMatrix44)  # 开始变换点云
        # then, we need to rotate all the camera poses
        train_cam_infos = transform_cams(train_cam_infos, MLMatrix44)  # 变换相机
...
# 修改了部分函数并去掉了 "Blender" : readNerfSyntheticInfo
