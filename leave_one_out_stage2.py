...
def leave_one_out_training(args, dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, train_id):
    ...
    #NOTE 使用有densitify的模型 且没有经过leave-one图片梯度的模型
    scene = Scene(dataset, gaussians, shuffle=False, extra_opts=args)  # make sure we load "densify_until_iter" model
    gaussians.training_setup(opt)
    if checkpoint:
        #NOTE restore上一步得到的6000迭代后的stage1 GS
        # make sure we load "densify_until_iter" model
    ...
    for iteration in range(first_iter, opt.iterations + 1):  # [6000 ~ 10000]
        ...
        if not viewpoint_stack:
            #NOTE 和stage1不一样 这里的leave-one图片完全不参与梯度回传
        ...
        with torch.no_grad():
            ...
            if iteration < opt.densify_until_iter:
                #NOTE 此时已经不再densitify了 因为第一个轮次就是 >= opt.densify_until_iter
    ...
    cur_status = gaussians.cache  #NOTE pure leave-one GS Model for 10000 epochs
    pre_status = torch.load(os.path.join(args.model_path, "gaussians_cache.pth"))  #NOTE sec 3.4 G_c^i_bar
    diffs = {}
...
def prepare_output_and_logger(args):
    ...
    # Set up output folder and dump cfg_args
...
def train_3dgs(args, ids):
    print(STR_STAGE + f"Optimizing {args.model_path}")
    ...
    for num_id, image_id in zip(range(args.sparse_view_num), ids):  # num_id: leave one out id, image_id: the id of the image to be infered
        ...
        #NOTE 这里还有设置参数的代码！这写法不太好。。
        #NOTE 设置成使用stage1训练了6000轮的模型
        args.start_checkpoint = os.path.join(args.model_path, "chkpnt6000.pth")  # load this ckpt
        # os.makedirs(args.model_path, exist_ok=True)  stage1已经makedirs了
