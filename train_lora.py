...
from utils.util_print import STR_DEBUG, STR_STAGE, STR_WARNING, STR_VERBOSE
...
if __name__ == '__main__':
    ...
    parser.add_argument('--sd_locked', action='store_true', default=False)  # True
    parser.add_argument('--only_mid_control', action='store_true', default=False)  # False
    parser.add_argument('--train_lora', action='store_true', default=False)  # True
    ...
    tstp = time.time()
    print(STR_STAGE, "begin to create model.......")
    model = create_model(f'./models/{args.model_name}.yaml').cpu()
    print(STR_STAGE, f"Model Created! Time comsuming: {time.time() - tstp} s.")
    model.load_state_dict(load_state_dict('./models/v1-5-pruned.ckpt', location='cpu'), strict=False)  # controlnet ckpt
    model.load_state_dict(load_state_dict(f'./models/{args.model_name}.pth', location='cpu'), strict=False)  # controlnet pth
    ...
    print(STR_WARNING, f"Loading batch size {args.batch_size} data...")
    # Setting numworkers=0 from https://github.com/pytorch/pytorch/issues/40403 to avoid the error
    # Because the code mixed the CPU operation and GPU operation inside the dataloader procedure
    ...
    tstp = time.time()
    print(STR_WARNING, "Begin to Train LoRA ... ...")
    trainer.fit(model, dataloader)
    print(STR_WARNING, f"Train LoRA Finished, time consumed : {time.time() - tstp} s")