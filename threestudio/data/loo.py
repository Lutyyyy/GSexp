...
class LooDataset(Dataset):
    def __init__(self, cfg: LooDataModuleConfig, split: str = 'train', sparse_num: int = 0):
        ...
        self.resolution = self.cfg.resolution  # 4
        self.sparse_num = sparse_num  # 4
        self.length = self.cfg.length  # 40000
        ...
        if self.sparse_num != 0:
            if self.split == 'train':
                ...
                with open(os.path.join(self.data_dir, f'sparse_test.txt')) as f:  # test: 35 / 279
                    self.sparse_ids = sorted([int(id) for id in f.readlines()])
        ...
        if self.resolution in [1, 2, 4, 8]:
            ...
            if not os.path.exists(tmp_images_folder):
                ...
                threestudio.info(f"threestudio/data/loo.py: Using resized images in {tmp_images_folder}...")
        ...
        # self.Rs, self.Ts 存的是所有的训练图片 也就是sparse_train的4/6/9张
        ...
        all_Rs, all_Ts , cam_c = [], [], []  # 存的是全部的279张图片
    ...
    def refresh_random_poses(self):
        # 根据GT的位姿和距离参数 生成self.refresh_size大小的随机位姿
        self.random_poses = []
        dis_from_gt = 0.8
        threestudio.info(f'threestudio/data/loo.py: refresh random poses with dis_drom_gt={dis_from_gt} at step {self.cnt}')
        self.random_poses = []
        while len(self.random_poses) < self.refresh_size:
            self.random_poses.extend(self.camera_sampler.sample_away_from_gt(dis_from_gt))  # GT图像就是sparse_train extend一次是4/6/9个
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        if self.split == 'train':
            idx = random.randint(0, len(self.sparse_ids) - 1)  # [0, 3]
            random_index = random.randint(0, self.refresh_size - 1)  # [0, 7]
            if self.cnt < self.around_gt_steps: # 在2800个样本前都执行
                if self.cnt % self.cfg.refresh_interval == 0: # 每训练完200个样本刷新位姿 包括清空random_poses 然后把away_from的采样positions加进去 每次生成8个位姿
                    self.refresh_random_poses()
                random_R, random_T = self.random_poses[random_index]  # 从那8个position样本里面挑一个出来
            else:
        ...
        ret = {
            "index": idx,  # sparse_train idx 或 sparse_test idx
            "R": self.Rs[idx],
            "T": self.Ts[idx],
            ...
            "txt": self.cfg.prompt,
            "random_index": random_index,  # [0, 7]随机的位姿或者idx本身 随机位姿只在训练模式下有用
            "random_R": random_R,  # 随机位姿对应的R
            "random_T": random_T,  # 随机位姿对应的T
            "random_poses": self.random_poses,  # 长度为refresh_size的随机位姿list
