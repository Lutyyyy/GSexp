...
def transform_poses_pca(poses: np.ndarray) -> Tuple[np.ndarray, np.ndarray, float]:
    ...
    """
    # 把所有相机移动到以相机的平均中心 本质就是所有相机全部移到世界坐标系原点为中心的区域
    t = poses[:, :3, 3]
...
class RandomCameraSampler:
    def __init__(
        ...
        # GaussianObject/utils/camera_utils.py
        gt_poses = np.array([np.linalg.inv(getWorld2View2(R, T))[:3, :4] for R, T in zip(Rs, Ts)])  # 拿到原生的相机位姿
        gt_poses[:, :, 1:3] *= -1  # YZ轴反向
        gt_poses, self.transform, self.scale_factor = transform_poses_pca(gt_poses)  # 用PCA归一化位姿
        ...
        # GaussianObject/utils/camera_utils.py/generate_ellipse_path_from_poses()
        # 注意区分场景中心和世界中心！
        self.center = focus_point_fn(poses)  # 拿到场景的中心
        offset = np.array([self.center[0], self.center[1], 0])  # 场景中心在2D上的投影

        sc = np.percentile(np.abs(poses[:, :3, 3] - offset), 100, axis=0)  # 相机距离场景中心的距离最大值 (x_max, y_max, z_max)
        # 拿到场景的两个最远点范围
        self.low = -sc + offset  # 短轴: y  场景中心2D投影同时减掉一个距离  (x0, y0, z0)
        self.high = sc + offset  # 长轴: x  场景中心2D投影同时加上一个距离  (x1, y1, z1)
        self.z_low = np.percentile((poses[:, :3, 3]), 0, axis=0)  # 距离世界中心最近的点  (x_n, y_n, z_n)
        self.z_high = np.percentile((poses[:, :3, 3]), 100, axis=0)  # 距离世界中心最远的点 (x_f, y_f, z_f)

        avg_up = poses[:, :3, 1].mean(0)
        avg_up = avg_up / np.linalg.norm(avg_up)
        ind_up = np.argmax(np.abs(avg_up))
        self.up = np.eye(3)[ind_up] * np.sign(avg_up[ind_up])
        # 以上是计算好了xyz轴的采样空间 以及up向量 用于生成环形位姿

        self.gt_thetas = []
        # 对归一化好的gt_poses进行循环 计算出每个姿态对应的角度值 也就是计算出get_position中的theta(但是是反向操作 get_positions是通过theta算position 这里是position算theta)
        for pose in gt_poses:
            pos = pose[:3, 3]
            # eq. ((y - y0) / (y1 - y0 - 0.5)) / ((x - x0) / (x1 - x0 - 0.5))
            theta = np.arctan(((pos[1] - self.low[1]) / (self.high[1] - self.low[1]) - 0.5) / ((pos[0] - self.low[0]) / (self.high[0] - self.low[0]) - 0.5))
            if pos[0] < self.center[0]:  # 如果x坐标在中心点左边 theta + 180° 也就是把theta角翻到右边
                theta += np.pi
            if theta < 0:  # 如果theta角为负 则 + 2π 保证为正
                theta += 2 * np.pi
            self.gt_thetas.append(theta)
        self.gt_thetas = sorted(self.gt_thetas)  # 返回的是xy-plane上的theta角度

    def get_position(self, theta: float):
        return np.array([
            self.low[0] + (self.high - self.low)[0] * (np.cos(theta) * .5 + .5),  # x_0 + (x1 - x0) * (cos(theta) / 2 + 0.5)
            self.low[1] + (self.high - self.low)[1] * (np.sin(theta) * .5 + .5),  # y_0 + (y1 - y0) * (sin(theta) / 2 + 0.5)
            self.z_variation * (self.z_low[2] + (self.z_high - self.z_low)[2] * (np.cos(theta + 2 * np.pi * self.z_phase) * .5 + .5)),  # A*(z_n+(z_f-z_n)*(cos(theta+2π*phase)/2+0.5))
        ])

    def sample_around_gt(self, dis_from_gt: float = 1.0):
        # 生成一组around的theta 然后根据theta生成一组位姿
        samples = []  # 长度为sparse_train == 4/6/9
        for theta_idx in range(len(self.gt_thetas)):  # 遍历所有gt的姿态
    ...
    def sample_away_from_gt(self, dis_from_gt: float = 1.0):
        # 生成一组away的theta 然后根据theta生成一组位姿
        samples = []
        for theta_idx in range(len(self.gt_thetas)):
            ...
            if np.random.randint(0, 2) == 0:
                ...
                prefix_theta = theta
            theta = (prefix_theta + suffix_theta) / 2  # 计算中间角度
            # 根据角度和距离参数随机均匀采样
            theta = np.random.uniform(theta - (theta - prefix_theta) / 2 * dis_from_gt, theta + (suffix_theta - theta) / 2 * dis_from_gt)
            if theta < 0:
                theta += 2 * np.pi
